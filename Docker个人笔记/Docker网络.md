...
`-A POSTROUTING -s 172.1.167.0/24 ! -o docker0 -j MASQUERADE`
...
3. NAT将源地址换成eth的IP：10.25.73.60->10.25.80.224
4. ping包从eth发送出去，到达10.25.80.224
...
dockerk可将容器对外提供服务的端口映射到host的某个端口上，外网通过该端口访问容器，容器启动时通过-p参数映射端口，如下如所示：

![映射端口1](/assets/映射端口-1.PNG)
...
![映射端口1](/assets/映射端口-2.PNG)

每一个映射的端口，host 都会启动一个 docker-proxy 进程来处理访问容器的流量：

![映射端口1](/assets/映射端口-3.PNG)

以 0.0.0.0:32775->8080/tcp 为例分析整个过程：

1. docker-proxy 监听 host 的 32775 端口
2. 当 curl 访问 10.25.73.60:32775 时，docker-proxy 转发给容器 172.1.167.3:80
3. httpd 容器响应请求并返回结果